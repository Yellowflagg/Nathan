# Fiche du bien — à compléter

> Ce fichier conditionne tous les chiffrages. Chaque champ vide est un chiffrage bloqué ou approximatif.
> Remplace les `?` au fur et à mesure. Ne supprime pas les lignes non renseignées : elles servent de liste de choses à aller chercher.

## Identification

| Champ | Valeur | Pourquoi c'est utile |
|---|---|---|
| Commune | **Castillon-la-Bataille (33350)** | PLU, PPRI, SPANC, aides locales |
| Adresse | **2, lieu-dit Robin** | interrogation géorisques, mairie |
| Intercommunalité | Communauté de communes Castillon–Pujols *(à confirmer)* | aides locales, OPAH éventuelle |
| Prix d'achat | 60 000 € | référence de rentabilité |
| Date d'acquisition | ? | délais de mise en conformité |
| Année de construction (approx.) | ? | > 15 ans = éligibilité aides ; > 2 ans = TVA 10 % |
| Type de construction | ? (pierre / parpaing / pisé / mixte) | traitement des murs, humidité |

## Surfaces

| Champ | Valeur |
|---|---|
| Surface habitable (m²) | **85 m²** |
| Emprise au sol (m²) | ? |
| Nombre de niveaux | ? |
| Nombre de pièces | ? |
| Combles (aménagés / perdus / aménageables) | ? |
| Terrain (m²) | ? |
| Dépendances | ? |

## Usage prévu — **détermine l'accès aux aides**

| Champ | Valeur |
|---|---|
| Résidence principale / locatif / secondaire | ? |
| Qui occupera le bien | ? |
| Sous quel délai | ? |
| Si locatif : conventionnement envisagé | ? |

## Situation fiscale du propriétaire — pour le calcul des aides

| Champ | Valeur |
|---|---|
| Revenu fiscal de référence | ? |
| Nombre de parts | ? |
| Catégorie de ressources (à déduire) | ? |

## Réseaux — état déclaré

| Réseau | État | À vérifier |
|---|---|---|
| Électricité | câbles coupés par endroits, à localiser | tableau, compteur, contrat Enedis actif ? |
| Eau | arrivées coupées par endroits | compteur, contrat, pression, matériau des canalisations |
| Évacuations / assainissement | ? | **tout-à-l'égout ou ANC ?** (mairie) |
| Chauffage | ? | énergie disponible, conduit de fumée existant ? |
| Gaz | ? | citerne, réseau, ou absent |
| Internet / fibre | ? | |

## Contraintes réglementaires — à faire remplir par `reno-reglementaire`

| Point | Statut |
|---|---|
| Classement PPRI (géorisques) | ? |
| Zonage PLU | ? |
| Périmètre ABF / monument historique | ? |
| Assainissement collectif ? | ? |
| Arrêté termites applicable | ? |

## État général déclaré

- Sol : **bon état** (à confirmer sur photos)
- Murs : **probablement à reprendre**
- Câbles électriques : **certains coupés**
- Arrivées d'eau : **certaines coupées**
- Toiture : ?
- Menuiseries : ?
- Humidité : ?

## Budget

| Champ | Valeur |
|---|---|
| Enveloppe disponible | ? |
| Étalement possible sur plusieurs années | ? |
| Capacité d'emprunt / éco-PTZ envisagé | ? |
| Temps disponible pour l'auto-rénovation (h/semaine) | ? |
| Compétences déjà maîtrisées | ? |
| Outillage déjà disponible | ? |

## Documents au dossier

- [ ] **DDT du notaire** (DPE, état parasitaire, élec, gaz, plomb, amiante) → déposer dans `00-contexte/`
- [ ] Plan cadastral
- [ ] Plan des niveaux (même à main levée)
- [ ] Titre de propriété / acte
- [ ] Photos complètes → `photos/`
